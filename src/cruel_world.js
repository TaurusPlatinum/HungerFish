...
class CruelWorld extends SafeWorld {
  ...
  next() {
    ...
    this.hunted = []
    super.next()
    if (this.T == CruelWorld.PIKE_TIME)
      this.add_predator()
    ...
    var preds = this.GM.get_vals().filter(c => c.kind == Wilds.PIKE)
  ...
  hunt(pred) {
    ...
    var preys = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.CRUCIAN)
  ...
  breed(u) {
    ...
    var preys = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.CRUCIAN)
    var preds = spots.map(p => this.GM.get(p)).filter(u => u && u.kind == Wilds.PIKE)
