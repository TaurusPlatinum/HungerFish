class ViewSettings {
  static CLR_BK = "CornflowerBlue"
  static CLR_CRUCIAN = "Chocolate"
  static CLR_PIKE = "DarkOliveGreen"
  static CLR_BK_CHART = "lightgray"
  static CLR_TEXT = "black"
  static CLR_LINE_CRUCIAN = "Chocolate"
  static CLR_LINE_PIKE = "DarkOliveGreen"
...
class ChartView {
  ...
  redraw(ctx) {
    ...
    if (this.data.length > 0 && this.data[0].length > 0) {
      ...
      var txt = "День: " + last_d.x + ", Карась: " + last_d.y + ", Щука: " + last_w.y
      ctx.fillText(txt, this.R.x + this.R.width / 2, this.R.y + this.R.height / 5);
    }

    this.draw_line(ctx, this.data[0], ViewSettings.CLR_LINE_CRUCIAN)
    this.draw_line(ctx, this.data[1], ViewSettings.CLR_LINE_PIKE)
...
class PredPreyView {
  ...
  draw_objects(ctx, data) {
    ...
    this.draw_herd(ctx, data, Wilds.CRUCIAN, ViewSettings.CLR_CRUCIAN)
    this.draw_herd(ctx, data, Wilds.PIKE, ViewSettings.CLR_PIKE)
  }
}
...
class PredPreyCtrl {
  ...
  constructor(view) {
    ...
    this.socket.on('state', function(msg){
      ...
      var preds = self.state.map.filter(a => a.kind == Wilds.PIKE)
